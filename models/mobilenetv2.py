"""
深度可分离卷积（Depthwise Separable Convolution）：将标准卷积分解为“逐通道卷积（Depthwise）”和“逐点卷积（Pointwise/1x1）”。
算力与精度的权衡：通过这种分解，计算量可以下降到标准卷积的 1/8 到 1/9，而精度只下降一点点。

V2 的核心贡献是提出了两个极其精妙的模块，彻底改变了轻量化网络的设计范式：
倒残差结构 (Inverted Residuals)：
ResNet 的残差：先 1x1 降维 -> 3x3 卷积 -> 1x1 升维。（像压缩饼干，先压实在处理）
V2 的倒残差：先 1x1 升维 -> 3x3 深度卷积 (Depthwise) -> 1x1 降维。（像发面，先膨胀开让特征充分交互，再压缩回去）
为什么这么做？ 因为深度卷积（Depthwise）只在单个通道内计算，没有通道间的信息交互。必须先通过 1x1 卷积把通道数“撑大”，特征交互才够充分。
...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        self.features = nn.Sequential(*features) # 19 layers
...
def mobilenet_v2(
    ...
    **kwargs: Any
    ...
    if weights is not None and _ovewrite_named_param is not None:
