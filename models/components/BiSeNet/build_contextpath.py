...
class resnet18(torch.nn.Module):
    def __init__(self, pretrained=True, weights=True):
        super().__init__()
...
class resnet50(torch.nn.Module):
    def __init__(self, pretrained=True, weights=True):
        super().__init__()
        self.features = models.resnet50(weights=weights) # pretrained=pretrained
        self.conv1 = self.features.conv1
        self.bn1 = self.features.bn1
        self.relu = self.features.relu
        self.maxpool1 = self.features.maxpool
...
def build_contextpath(name):
    model = {
        'resnet18': resnet18(weights=True), # pretrained=True
        'resnet50': resnet50(weights=True),
        'resnet101': resnet101(weights=True)
    }
    return model[name]
