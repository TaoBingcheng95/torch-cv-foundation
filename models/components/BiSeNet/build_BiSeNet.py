...
class Spatial_path(torch.nn.Module):
    def __init__(self, in_channels:int=3):
        super().__init__()
        self.convblock1 = ConvBlock(in_channels=in_channels, out_channels=64)
...
class BiSeNet(torch.nn.Module):
    def __init__(self, in_channels:int=3, num_classes=10, context_path= 'resnet18'):
        super().__init__()
        # build spatial path
        self.saptial_path = Spatial_path(in_channels=in_channels)
        ...
        if context_path == 'resnet101':
            ...
            # build feature fusion module
            # resnet50 1792 = 256(from context path) + 512(from spatial path) + 1024(from spatial path)
            self.feature_fusion_module = FeatureFusionModule(num_classes, 3328)
    ...
    def forward(self, inputs):
        ...
        # feat8, feat16, feat32, tail 
        _, cx1, cx2, tail = self.context_path(inputs)
...
if __name__ == '__main__':
    ...
    model = BiSeNet(in_channels=3, num_classes=32, context_path='resnet50')
    model.eval()
