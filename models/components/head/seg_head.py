...
class GenericSegmentationHead_1(nn.Module):
    def __init__(self, in_channels, num_classes, target_resolution=(224, 224)):
        ...
        """
        super(GenericSegmentationHead_1, self).__init__()
        ...
        while True:
            ...
            self.upsample_layers.append(
                nn.ConvTranspose2d(current_channels, current_channels // 2,
                                   kernel_size=4, stride=2, padding=1)
            )
            self.conv_layers.append(
                nn.Sequential(
                    nn.Conv2d(current_channels // 2, current_channels // 2,
    ...
    def forward(self, x, input_resolution):
        ...
        if current_H != target_H or current_W != target_W:
            x = F.interpolate(x, size=self.target_resolution,
