...
class CIFAR10Loader:
    ...
    def _init_ds(self):
        self.train_ds = CIFAR10(root=self.root,
                                train=True,
                                transform=self.train_transform,
                                download=self.download)
        self.test_ds = CIFAR10(root=self.root,
                               train=False,
        ...
        if self.val_split:
            train_count = len(self.train_ds)
            self.train_ds, self.val_ds = random_split(self.train_ds,
                                                      [train_count - int(train_count * self.val_split),
                                                       int(train_count * self.val_split)])


    def loader(self, batch_size=4, num_workers=0):
        self.train_loader = DataLoader(
    ...
    def plot_sample(self):
        x, y = self.val_ds[0]
        fig, axis = plt.subplots()
        axis.imshow(x.permute(1, 2, 0)/ 2 + 0.5) # unnormalize
        axis.set_title(self.idx_to_class[y])
        plt.show()
