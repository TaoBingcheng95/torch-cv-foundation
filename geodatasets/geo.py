...
"""Base classes for all :mod:`torchgeo` geodatasets."""
...
from geodatasets.errors import DatasetNotFoundError
from geodatasets.utils import (
    BoundingBox,
    Path,
    array_to_tensor,
    concat_samples,
...
class GeoDataset(Dataset[dict[str, Any]], abc.ABC):
    """Abstract base class for geodatasets containing geospatial information.
    ...
    * resolution
    ...
    :class:`GeoDataset` is a special class of geodatasets. Unlike :class:`NonGeoDataset`,
    the presence of geospatial information allows two or more geodatasets to be combined
    ...
    * Combine image and target labels and sample from both simultaneously
      (e.g., Landsat and CDL)
    * Combine geodatasets for multiple image sources for multimodal learning or data fusion
    ...
    * Combine image and other raster data (e.g., elevation, temperature, pressure)
    ...
    These combinations require that all queries are present in *both* geodatasets,
    ...
    * Combine geodatasets for multiple image sources and treat them as equivalent
      (e.g., Landsat 7 and Landsat 8)
    * Combine geodatasets for disparate geospatial locations
    ...
    #: other geodatasets. It should not include a file extension, as the dataset may be in
...
class RasterDataset(GeoDataset):
    ...
    #:
    #: For geodatasets with both model inputs and outputs, a custom
...
class NonGeoDataset(Dataset[dict[str, Any]], abc.ABC):
    """Abstract base class for geodatasets lacking geospatial information.

    This base class is designed for geodatasets with pre-defined image chips.
    """
...
class NonGeoClassificationDataset(NonGeoDataset, ImageFolder):  # type: ignore[misc]
    """Abstract base class for classification geodatasets lacking geospatial information.

    This base class is designed for geodatasets with pre-defined image chips which
...
class IntersectionDataset(GeoDataset):
    ...
    * Combine image and other raster data (e.g., elevation, temperature, pressure)
    ...
    def __init__(
        ...
        When computing the intersection between two geodatasets that both contain model
        ...
        Raises:
            RuntimeError: if geodatasets have no spatiotemporal intersection
    ...
    def _merge_dataset_indices(self) -> None:
        """Create a new R-tree out of the individual indices from two geodatasets."""
        i = 0
    ...
    def __getitem__(self, query: BoundingBox) -> dict[str, Any]:
        ...
        # All geodatasets are guaranteed to have a valid query
...
{self.__class__.__name__} Dataset
    ...
    @property
    def crs(self) -> CRS:
        """:term:`coordinate reference system (CRS)` of both geodatasets.
    ...
    @crs.setter
    def crs(self, new_crs: CRS) -> None:
        """Change the :term:`coordinate reference system (CRS)` of both geodatasets.
    ...
    @property
    def res(self) -> float:
        """Resolution of both geodatasets in units of CRS.

        Returns:
            Resolution of both geodatasets.
        """
        return self._res

    @res.setter
    def res(self, new_res: float) -> None:
        """Change the resolution of both geodatasets.
...
class UnionDataset(GeoDataset):
    ...
    * Combine geodatasets for disparate geospatial locations
    ...
    def __init__(
        ...
        When computing the union between two geodatasets that both contain model inputs
    ...
    def __getitem__(self, query: BoundingBox) -> dict[str, Any]:
        ...
        # Not all geodatasets are guaranteed to have a valid query
        samples = []
        for ds in self.datasets:
...
{self.__class__.__name__} Dataset
    ...
    def res(self) -> float:
        ...
        Returns:
            The resolution of both geodatasets.
        """
        return self._res
