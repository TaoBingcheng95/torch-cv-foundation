...
from ..geodatasets import BoundingBox, GeoDataset
from .constants import Units
from .utils import _to_tuple, get_random_bounding_box, tile_to_chips


class GeoSampler(Sampler[BoundingBox], abc.ABC):
    """Abstract base class for sampling from :class:`~torchgeo.geodatasets.GeoDataset`.
    ...
    :class:`~torchgeo.geodatasets.GeoDataset`. This includes things like latitude,
...
class RandomGeoSampler(GeoSampler):
    ...
    This sampler is not recommended for use with tile-based geodatasets. Use
...
class PreChippedGeoSampler(GeoSampler):
    ...
    This is particularly useful for geodatasets that contain geospatial metadata
    and subclass :class:`~torchgeo.geodatasets.GeoDataset` but have already been
    pre-processed into :term:`chips <chip>`.

    This sampler should not be used with :class:`~torchgeo.geodatasets.NonGeoDataset`.
    ...
    :class:`~torchgeo.geodatasets.IntersectionDataset`, or when each file is in a
