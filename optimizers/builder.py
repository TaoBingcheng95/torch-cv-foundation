...
def _build_param_groups(model: nn.Module, lr: float, weight_decay: float,
...
def build_scheduler(optimizer: optim.Optimizer,
    ...
    if sched_type == "warmup_cosine":
        ...
        sched_total_epochs = cfg.get("total_epochs", total_epochs)
        eta_min = cfg.get("eta_min", 1e-6)  # 最小学习率绝对值

        # 为每个参数组构建独立的 lambda，保证 eta_min 作为绝对值时的精确等价
        #   backbone 组: base_lr=1e-4 → eta_min_ratio = 1e-6/1e-4 = 0.01
        #   head 组:     base_lr=3e-4 → eta_min_ratio = 1e-6/3e-4 ≈ 0.0033
        lambdas = []
        for pg in optimizer.param_groups:
            ...
            lambdas.append(
                partial(
                    ...
                    total_epochs=sched_total_epochs,
